package handlers

import (
	"context"
	"encoding/json"
	"net/http"
	"time"

	"github.com/gorilla/mux"
	"google.golang.org/api/iterator"
	"task.com/orderManagement/firebase"
	"task.com/orderManagement/models"
)

func GetDiariesOrdersEmployee(w http.ResponseWriter, r *http.Request) {
	// Obtener el companyId y employeeId de la URL
	vars := mux.Vars(r)
	companyId := vars["companyId"]
	employeeId := vars["employeeID"]

	// Obtener la fecha actual en UTC
	currentDate := time.Now().Format("2006-01-02")

	// Realizar una consulta en Firestore para buscar las órdenes del día actual y de la empresa específica
	iter := firebase.Client.Collection("order").Where("CompanyId", "==", companyId).Where("Date", "==", currentDate).Documents(context.Background())
	var orders []models.Order
	for {
		doc, err := iter.Next()
		if err == iterator.Done {
			break
		}
		if err != nil {
			http.Error(w, "Error al obtener las órdenes", http.StatusInternalServerError)
			return
		}
		var order models.Order
		doc.DataTo(&order)

		// Verificar si el employeeId está en la lista de empleados de la orden
		for _, employee := range order.Employees {
			if employee.ID == employeeId {
				orders = append(orders, order)
				break
			}
		}
	}

	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(orders)
}
